...
export async function onRequestPost(context) {
  ...
  if (!ALLOWED_TYPES.has(type)) {
    return json({ error: "Unsupported divination type." }, 400);
  }

  if (!question) {
    return json({ error: "A question is required for AI interpretation." }, 400);
  }

  if (!result || typeof result !== "object") {
    return json({ error: "Result payload is required." }, 400);
  }
  ...
  try {
    ...
    return json({ error: "The AI interpretation could not be generated right now." }, 502);
  }
}
